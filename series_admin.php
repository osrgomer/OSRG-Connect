...
if (!isset($_SESSION['user_logged_in']) || $_SESSION['user_logged_in'] !== true) {
    header('Location: series_login.php');
    exit;
}
...
<body>
    ...
    <div class="container mx-auto px-4 py-8 <?php echo isset($_SESSION['admin_origin']) ? 'mt-12' : ''; ?>">
        ...
        <div class="text-center mb-8">
            ...
            <a href="series_index.php" class="text-sm text-slate-400 hover:text-cyan-400">
    ...
    <script>
        ...
        async function loadStats() {
            try {
                const response = await fetch('api_series_admin.php?action=get_stats');
        ...
        async function impersonate(userId, username) {
            ...
            try {
                const response = await fetch('api_series_admin.php?action=impersonate', {
                    method: 'POST',
                ...
                if (data.success) {
                    window.location.href = 'series_index.php';
                }
            } catch (error) {
        ...
        async function exitImpersonate() {
            try {
                await fetch('api_series_admin.php?action=exit_impersonate');
        ...
        document.getElementById('uploadForm').onsubmit = async (e) => {
            ...
            try {
                const response = await fetch('series_api_admin_uploads.php', { method: 'POST', body: formData });
        ...
        async function loadUploads() {
            try {
                const response = await fetch('series_api_admin_uploads.php');
                const data = await response.json();
